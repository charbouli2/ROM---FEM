function [ux,v,p,SNAP_cluster,SNAP] = SVDtotal(COOR_v,CN_v,rnod_v,dR_v,COOR_p,rnod_p,dR_p,K,G,res,...
...
%   - p: vector with the pressure
    ...
    icluster=1;
    SNAP.X = zeros(totDOF_l,steps); %% Edited : Changed the dimension (para solo calcular SVD de no velocidades no conocidas: Numero de grados de libertad dl)
    ...
    VAR.X = zeros(totDOF_l,1); %% Edited : Changed the dimension (para solo calcular SVD de no velocidades no conocidas: Numero de grados de libertad dl)
    ...
    DATA.STORE.TOLERANCE_SVD_COMPRESSION.X=tolU;
    ...
    istep=2; % $$ Hay que mirar si esto esta bien o tiene que ser istep = 1 $$
    time1=tic;
    while istep<=steps
        dR_v=dR_v+deltadRv;
        u=u+deltaU;
        ...
        % Decomposition of the different terms
         ...
         u = d(1:size(Kll,1)) ; % Ajouter by moi, juste pour extraire le necessaire
        ...
        % VAR.p=pl; % Modifié by me, tu as mis pl a la place de p
